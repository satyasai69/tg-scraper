...
async function scrapeTelegramGroups(keyword) {
    ...
    try {
        for (let pageNum = 1; pageNum <= 500; pageNum++) {
            const url = `https://en.tgramsearch.com/search?query=${encodeURIComponent(keyword)}&page=${pageNum}`;
            ...
            try {
                ...
                if (match) {
                    if (match[1]) {
                        ...
                        if (realPage < pageNum) {
                            console.log(`⚠️ Tried to open page ${pageNum}, but got redirected to page ${realPage}. Stopping.`);
                            break;
                        }
                    }
                }
                const joinLinks = await page.$$eval(".tg-channel-wrapper.is-list a[href*='/join/']", (anchors) => anchors.map((a) => a.href));
                if (joinLinks.length === 0) {
                    console.log(`⚠️ No results found on page ${pageNum}, stopping.`);
                    break;
                }
                console.log(`➡️ Found ${joinLinks.length} join links on page ${pageNum}`);
                for (const joinUrl of joinLinks) {
                    ...
                    try {
                        ...
                        const channel = await page.evaluate(() => {
                            ...
                            const linkEl = document.querySelector("a[href^='tg://resolve?domain=']");
                            const link = linkEl ? linkEl.getAttribute("href") || "" : "";
                            let username = "";
                            const m = link.match(/domain=([^&]+)/);
                            if (m)
                                username = m[1] || ""; // Ensure username is a string
...
async function saveToJSON(data, filename = "telegram_channels.json") {
    try {
